  ...
  return (
    <div className=" mx-14 mt-8 p-4 border border-gray-300 rounded-lg shadow-lg bg-white">
      ...
      <div className=" mt-4">
        ...
        <p className="text-gray-600 mt-2">{data?.locality}, {data?.locality}</p>
        <span>{data?.avgRating}</span>
      <p className="text-sm text-gray-600 mt-2">{data?.cuisines}</p>
      </div>
        <img src={Url + data?.cloudinaryImageId} alt="Image not found" className=" w-[20%]  object-cover rounded-t-lg" />
